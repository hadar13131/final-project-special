...
def main() -> None:
    ft.app(target=Add_Workout.main)
...
class Add_Exercise:
    ...
    # def go_to_add_set(self, e: ft.ControlEvent):
    ...
    def click(self, e: ft.ControlEvent):
        ...
        w1 = self.workout["exercises"]
        e1 = json.loads(w1)
        e1 = w1
        for s in e1:
            if s["name"] == self.name1:
                self.sets_lst = s["sets"]

        lst_sets_control = ShowTheWorkout.format_set_lst(self.sets_lst)
